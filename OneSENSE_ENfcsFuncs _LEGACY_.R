...
prepFcsFolderData <- function(LoaderPATH=LoaderPATH, ceil = ceil, useCSV = useCSV){
  if(!useCSV){
    ...
    csvdata <- lapply(paste0(LoaderPATH,"//",csvfilenames),function(x) read.csv(x, check.names = F,stringsAsFactors = FALSE))
    FFdata<-NULL
...
FCStSNEone <- function (LoaderPATH ="fcs", #Folder with samples (even 1 sample ok)
  ...
  if(DotSNE)
  {
    print(dim(data1))
    print("running tSNE")
    tSNEmat <- Rtsne(data1, dims=2, perplexity=tSNEperplexity, check_duplicates=F,verbose = T)$Y
  ...
  if(DoOneSENSE)
  {
    ...
    for (factor in 2:(dim(keeptable)[2])){
      ...
      tSNEdata3 <- Rtsne(dataX, dims=1, check_duplicates=F,verbose = T,perplexity = tSNEperplexity) 
      ...
      OneStSNEmat <- cbind(Xx1DtSNEmat,tSNEmat1) #bug: what is xx1dtsnemat
    }
  ...
  if(Do3DtSNE)
  {
    ...
    TDtSNEdata3 <- Rtsne(data1, dims=3, perplexity=tSNEperplexity, check_duplicates=F,verbose = T )
...
meaningPlot <- function (LoaderPATH =sourceFcsForMeaningPlot,
  ...
  tempdata <- data[, which (colnames(data) %in% keeprows[,1])]
  
  rasters=sapply(colnames(tempdata),function(pname)
  {
    Exp <- data[,pname]
...
{  
  if (!require(gplots)) { # using heatmap.2 function
    install.packages("gplots", dependencies=TRUE, repos="http://cran.cnr.Berkeley.edu")
  }
  if (!require(flowCore)) { # using logicleTransform
    source("http://bioconductor.org/biocLite.R")
    biocLite("flowCore")
...
# Returns a set of data with
gatingDataPoints <- function(useUMAP, LoaderPATH, ceil, useCSV, cropped, gatedPATH) {
  library(hypergate)
  ...
  prepData <- prepFcsFolderData(LoaderPATH = filePATH, ceil = ceil, useCSV = useCSV)
  ...
  labels <- NULL
  if (useUMAP) {
    labels <- c(paste0("UMAP1", gatedModifier), paste0("UMAP2", gatedModifier))
  } else {
    labels <- c(paste0("tSNE1", gatedModifier), paste0("tSNE2", gatedModifier))
  }
  ...
  gatedPoints <- gate_from_biplot(dataMat, labels[1], labels[2], xlim = c(0, 10000), ylim = c(0, 10000), sample = 20000)
  gatedDataBool <- sapply(gatedPoints, function(x) (!is.na(x)))
  counter <- 0
  ...
  NewTransformBase <- c (rep(1, dim(FFdata)[2]-1))
  if(TransformOutput){
    NewTransformNew <- c(rep(2, dim(score2)[2]),
                         rep(3, 1))
  }else  NewTransformNew <- c(rep(3, dim(score2)[2]),
                              rep(3, 1))
  NewTransform <- c(NewTransformBase,NewTransformNew)
  ...
  FFdata <- cbind(FFdata[, 1:(ncol(FFdata) - 2)],
                  NIscore,
                  FFdata[, (ncol(FFdata) - 1):ncol(FFdata)])
  
  if (cropped) {
    FFdata <- FFdata[gatedDataBool, ]
  }
  
  plot(FFdata[, labels[1]], FFdata[, labels[2]], xlab = labels[1], ylab = labels[2], xlim = c(0, 10000), ylim = c(0, 10000))
  
  if(!useCSV){
    for (FFs in 1:NumBC){
      suppressWarnings(dir.create(paste0(LoaderPATH,"_",gatedModifier)))
      
      newFF <- forNewFF
      newBaseData <- FFdata[FFdata[,ncol(FFdata)]==FFs,-(c(grep(labels[1], colnames(FFdata)),
                                                           grep(labels[2], colnames(FFdata)),
                                                           ncol(FFdata)))
                            , drop = FALSE]
      subsetNIScore <- cbind(FFdata[, labels[1]], FFdata[, labels[2]])[FFdata[,dim(FFdata)[2]]==FFs,,drop=FALSE]
      colnames(subsetNIScore) <- c(labels[1], labels[2])
      newBaseDataDesc <- colnames(newBaseData)
      colnames(newBaseData) <- OrigNames
      exprs(newFF) <- newBaseData
      newFF <- cbind2(newFF, subsetNIScore)
      ...
      BaseFN <- sapply(strsplit(sapply(strsplit(FcsFileNames[FFs], split ="\\."), "[", 1), split = "_"), "[")
      BaseFN <- paste0(BaseFN[1:(length(BaseFN) - 1)], collapse = "_")
      FNresult <- paste0(LoaderPATH,"_", gatedModifier, "/",BaseFN,"_",gatedModifier,".fcs")
      newFF@description$'$FIL' <- paste0(BaseFN,"_",gatedModifier,".fcs")
      ...
      newFF@description$CREATOR <- "ENewellScript"
      newFF@description$FILENAME <- paste0(BaseFN,"_",gatedModifier,".fcs")
      identifier(newFF) <- paste0(BaseFN,"_",OutputSuffix)
    ...
    suppressWarnings(dir.create(paste0(LoaderPATH,"_csv")))
    for (FFs in 1:NumBC){
